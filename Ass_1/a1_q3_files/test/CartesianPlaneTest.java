...
//        for (int j = 5; j >= 0; --j) {
//            for (int i = 0; i <= 5; ++i) {
//                System.out.print(plane.get(i,j) + " ");
//            }
//            System.out.println();
//        }
//        System.out.println();
...
//        }
    ...
    @Test(expected = IndexOutOfBoundsException.class)
    public void testOutOfBoundsVsNegativeCoordinates() {
        plane.get(-2, -3);
    }

    @Test
    public void testResizeNegativeBounds() {
        plane.resize(-2, 2, -3, 0);
        if (plane.get(-2,-3) != null) {
            fail();
        }
    }

    @Test
    public void testResizeNegativeBounds1() {
        plane.add(1, 0, 55);
        plane.add(2, 0, 99);
        plane.resize(-2, 2, -3, 0);
        if (plane.get(1,0) != Integer.valueOf(55)) {
            fail();
        }
        if (plane.get(2,0) != Integer.valueOf(99)) {
            fail();
        }
    }

    @Test(expected = IllegalArgumentException.class)
    public void testInvalidResizeNegtiveBounds() {
        plane.add(2, 1, 9);
        plane.resize(-2, 2, -3, 0);
    }
}
